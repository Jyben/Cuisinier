...
{
    /// <inheritdoc />
    public partial class MigrateMenusAndShoppingListsFromTestUserToSecondUser : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Migrate Menus, ShoppingLists, and Favorites from test user (migration@cuisinier.local) to the second user
            // Note: Dishes were already migrated in a previous migration
            ...
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    -- Migrate favorites from test user to second user
                    IF @FavoritesCount > 0
                    BEGIN
                        UPDATE Favorites
                        SET UserId = @SecondUserId
                        ...
                        PRINT 'Migrated ' + CAST(@FavoritesCount AS NVARCHAR(10)) + ' favorites from test user to second user.';
                    END

                    IF @MenusCount = 0 AND @ShoppingListsCount = 0 AND @FavoritesCount = 0
                    BEGIN
                        PRINT 'No menus, shopping lists, or favorites found to migrate from test user.';
                    END
                END
                ELSE
                BEGIN
        ...
        {
            // Reverse migration: move data back to test user
            ...
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    -- as we cannot determine which data originally belonged to the test user
